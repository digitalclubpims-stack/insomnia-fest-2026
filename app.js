...
function setupAnimations(){const reduce=matchMedia('(prefers-reduced-motion: reduce)').matches;if(reduce){$$('.reveal').forEach(e=>e.classList.add('visible'));return}if(window.gsap&&window.ScrollTrigger){gsap.registerPlugin(ScrollTrigger);$$('.reveal').forEach((el,i)=>{gsap.fromTo(el,{y:90,opacity:0,rotateX:7},{y:0,opacity:1,rotateX:0,duration:.68,ease:'power3.out',delay:Math.min(i*.035,.18),scrollTrigger:{trigger:el,start:'top 88%',once:true}})});$$('.category-card').forEach((c,i)=>{gsap.fromTo(c,{y:150,rotate:i%2?5:-5,scale:.94,opacity:0},{y:0,rotate:i%2?-1.15:1.15,scale:1,opacity:1,duration:.72,ease:'power4.out',scrollTrigger:{trigger:c,start:'top 92%',once:true}})});$$('.event-card').forEach((c,i)=>{gsap.fromTo(c,{y:165,rotate:i%2?9:-9,scale:.9,opacity:0},{y:0,rotate:i%2?-1.15:1.15,scale:1,opacity:1,duration:.72,ease:'power4.out',scrollTrigger:{trigger:c,start:'top 91%',once:true}});gsap.to(c,{y:i%2?-20:20,scrollTrigger:{trigger:c,start:'top bottom',end:'bottom top',scrub:.75}})});$$('.display').forEach(h=>gsap.fromTo(h,{scale:.75,y:80,opacity:0},{scale:1,y:0,opacity:1,ease:'power3.out',scrollTrigger:{trigger:h,start:'top 88%',end:'top 45%',scrub:1}}));$$('.section-head').forEach(h=>gsap.fromTo(h,{x:-80,opacity:0},{x:0,opacity:1,duration:1,ease:'power4.out',scrollTrigger:{trigger:h,start:'top 85%',once:true}}));$$('.night-card').forEach((c,i)=>gsap.fromTo(c,{y:115,rotate:i%2?5:-5,opacity:0},{y:0,rotate:i%2?-1:1,opacity:1,duration:.68,delay:i*.08,ease:'power3.out',scrollTrigger:{trigger:c,start:'top 90%',once:true}}));}else{const io=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting)e.target.classList.add('visible')}),{threshold:.12});$$('.reveal').forEach(e=>io.observe(e))}}
